...
1. الصفحة العامة `/` ثم صفحة اختيار طريقة الدخول `/login`.
...
| `parent` | `parent.hassan@ofuq.local` | `OfuqLocal123!` | `/portal` | بوابة متابعة الأبناء، الحضور، الدرجات، المالية، الإعلانات، ومراسلة المدرسة ضمن نطاق محدود وآمن |
| `student` | `student.youssef@ofuq.local` | `OfuqLocal123!` | `/portal` | بوابة الطالب، بياناتي، الدرجات، الجدول، المكتبة، والإعلانات مع إتاحة مراسلة المدرسة فقط |
...
| `/login` | موجود | صفحة اختيار نوع الحساب بين الموظفين والبوابة |
| `/login/staff` | موجود | دخول الموظفين والإدارة ضمن UX مخصص |
| `/login/portal` | موجود | دخول الطالب وولي الأمر ضمن UX مخصص |
| `/login/reset-password` | موجود | واجهة طلب إعادة التعيين فقط من دون إرسال بريد فعلي |
...
| `/portal/chat` | موجود | يمكن استخدامها لعرض مراسلة ولي الأمر أو الطالب مع إدارة المدرسة في المدرسة الحالية |
| `/portal/assistant` | موجود | صفحة تمهيدية لمساعد أُفُق داخل البوابة من دون Gemini فعلي |
| `/portal/profile` | موجود | الملف الشخصي وعضوية المدرسة |
| `/dashboard/chat` | موجود | يمكن استخدامها لعرض رد إدارة المدرسة على رسائل أولياء الأمور والطلاب بشكل مباشر |
| `/dashboard/assistant` | موجود | صفحة تمهيدية لمساعد أُفُق للموظفين من دون تنفيذ AI فعلي |
...
- انتقل إلى `/login` ثم اختر بطاقة الموظفين والإدارة.
...
- سجّل الخروج ثم ارجع إلى `/login` واختر بوابة الطالب وولي الأمر.
- سجّل الدخول بحساب `parent.hassan@ofuq.local` أو `student.youssef@ofuq.local`.
- افتح `/portal`.
- انتقل إلى الطلاب أو بياناتي، ثم الحضور، الدرجات، المالية، المكتبة، والإعلانات.
- أكّد بوضوح أن البوابة تبقى قراءة فقط في معظم الوحدات، مع استثناء مسار مراسلة المدرسة الداخلي فقط.
...
| `parent` | `/portal` | متابعة الأبناء، الحضور، الدرجات، المالية، الإعلانات، ومراسلة المدرسة ضمن نفس المدرسة |
| `student` | `/portal` | البيانات الشخصية، الدرجات، الجدول، المكتبة، الإعلانات، ومراسلة المدرسة ضمن نفس المدرسة |

## Known Limitations

- التكاملات الخارجية معروضة كإعدادات واجهة ضمن نطاق MVP من دون OAuth أو مزامنة أو اتصال فعلي.
- أزرار Google وصفحة إعادة التعيين ضمن تجربة الدخول الحالية هي واجهات محلية فقط، ولا تنفذ OAuth أو إرسال بريد فعلي.
- الذكاء الاصطناعي وواجهة الاستعلام الذكي مؤجلان إلى مرحلة لاحقة بعد توفر بيانات تشغيلية كافية.
- صفحات `/dashboard/assistant` و`/portal/assistant` ما تزال تأسيس واجهة وبنية فقط، ولا تنفذ Gemini في النسخة الحالية.
- يدعم `/dashboard/chat` و`/portal/chat` المراسلة الداخلية الفعلية فقط. لا توجد مرفقات، ولا تحرير أو حذف للرسائل، ولا إشعارات خارجية.
- بوابة ولي الأمر والطالب مخصصة للعرض والقراءة فقط في معظم الوحدات المحلية، مع بقاء مراسلة المدرسة هي مسار الكتابة الوحيد.
...
- [ ] اختيار بطاقة الموظفين والإدارة من `/login`.
...
- [ ] تسجيل الخروج ثم اختيار بوابة الطالب وولي الأمر من `/login`.
- [ ] تسجيل الدخول بحساب `parent.hassan@ofuq.local` أو `student.youssef@ofuq.local`.
- [ ] فتح `/portal`.
- [ ] التأكد أن روابط لوحة الإدارة غير ظاهرة داخل البوابة.
- [ ] التأكد أن صفحات البوابة تبقى قراءة فقط خارج `/portal/chat`.
