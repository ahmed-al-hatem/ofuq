...
| `/portal/assistant` | موجود | مساعد أُفُق يعمل فعليًا من الخادم فقط مع محفوظات محادثة ونطاق مرتبط ببيانات ولي الأمر أو الطالب |
...
| `/dashboard/assistant` | موجود | مساعد أُفُق يعمل فعليًا للموظفين مع ملخصات مقيدة بالدور والمدرسة الحالية |
...
- واجهة AI Query الأوسع ما تزال مؤجلة، لكن مساعد أُفُق الأساسي يعمل الآن من الخادم فقط وبنطاق قراءة محدود.
- يتطلب الرد الفعلي من المساعد ضبط `GEMINI_API_KEY` في بيئة التشغيل المحلية أو بيئة النشر؛ عند غياب المفتاح تظهر رسالة إعداد آمنة بدل التعطل.
